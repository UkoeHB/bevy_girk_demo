...
fn process_player_inputs(world: &mut World, handler: impl Fn(&mut World, PlayerClientInput))
{
    ...
    while let Some(input) = player_inputs.try_recv()
    {
        ...
        { tracing::warn!(?input, "ignoring input sent by a non-player client"); continue; }
...
{
    let PlayerClientInput::Init = input
...
{
    let PlayerClientInput::Prep = input
...
{
    let PlayerClientInput::GameOver = input
