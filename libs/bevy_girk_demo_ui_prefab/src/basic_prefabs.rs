...
) -> Entity
{
    ...
    let text_entity = ui
        .commands()
        .spawn(TextElementBundle::new(widget, text_params, initial_text))
        .id();
...
) -> Entity
{
    ...
    entity_commands.insert(TextElementBundle::new(
        button_overlay,
        TextParams::center()
...
/// Make a basic popup.
///
/// The popup is a square with cancel and accept buttons. The button text and behavior can be customized. Pressing
/// the cancel button will set the popup overlay visibility to `false`. You should set it to `true` manually in
/// order to activate the popup. Note that the popup starts with visiblity `false`.
///
/// The popup is attached to the `"root"` widget. This function will panic if the root of your main ui tree does
/// not contain a widget with that name.
///
/// The popup is arbitrarily set at a depth of `500.0` in order to supercede the normal UI tree. Multiple
/// concurrent popups will NOT stack on each other properly.
...
) -> BasicPopupPack
{
    ...
    window_overlay
        ...
        .unwrap()
        .get_container_mut()
        .set_render_depth(Modifier::Set(500.));
    ...
    ui.commands()
        .spawn((window_img, UiInteractionBarrier::<MainUi>::default()));
    ...
    let button_height = style.button_ratio * button_width * ((100. - style.button_dead_space) / 100.)
        / ((100. - content_percent) / 100.);
    let button_vertical_spacing = (100. - (button_height + style.button_dead_space)) / 2.;
    let (cancel_button, cancel_entity, accept_button, accept_entity) = ui.div(move |ui| {
        // add button style
        ...
        let cancel_entity = spawn_basic_button(ui, &cancel_button, cancel_text, move |world: &mut World| {
            syscall(
                world,
                (MainUi, [], [window_overlay_clone.clone()]),
