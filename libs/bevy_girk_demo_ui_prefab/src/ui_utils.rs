...
/// Toggle a button's availability by showing/hiding a widget that blocks the button, and toggling the button
/// label's font color.
pub fn toggle_button_availability<Ui: LunexUi>(
    In((enable, button_blocker, text_entity)): In<(bool, Widget, Entity)>,
...
)
{
    ...
    let Ok(mut ui) = uis.get_single_mut() else {
        tracing::error!("multiple uis detected in toggle button availability");
        return;
    };
    let Ok(branch) = button_blocker.fetch_mut(&mut ui) else {
        tracing::error!("button blocker widget is missing");
        return;
    };
...
{
    let Some(mut entity) = world.get_entity_mut(text_entity) else {
        tracing::error!("failed writing ui text, missing entity");
        return;
    };
    let Some(text) = entity.get_mut::<Text>() else {
        tracing::error!("failed writing ui text, entity missing Text component");
        return;
    };
...
impl<'w, 's, Ui: LunexUi> UiUtils<'w, 's, Ui>
{
    ...
    pub fn get_relative_layout_mut<'a, 'b: 'a>(&'a mut self, widget: &'b Widget)
        -> Option<&'a mut RelativeLayout>
    {
        let Ok(widget_branch) = widget.fetch_mut(self.builder.tree()) else {
            return None;
        };
        let LayoutPackage::Relative(ref mut layout) = widget_branch.get_container_mut().get_layout_mut() else {
            return None;
        };
    ...
    {
        !self.builder.tree().drop_branch(widget.path()).is_err()
    }
    ...
    pub fn toggle(&mut self, enable: bool, widget: &Widget)
    {
        ...
        let Ok(widget_branch) = widget.fetch_mut(ui) else {
            return;
        };
    ...
    )
    {
        ...
        for on_widget in on_widgets {
            let Ok(on_widget_branch) = on_widget.fetch_mut(ui) else {
                continue;
            };
        ...
        for off_widget in off_widgets {
            let Ok(off_widget_branch) = off_widget.fetch_mut(ui) else {
                continue;
            };
    ...
    pub fn toggle_basic_button(&mut self, enable: bool, text_entity: Entity, button_blocker: &Widget)
    {
        // toggle visibility of button blocker
        let ui = self.builder.tree();
        let Ok(branch) = button_blocker.fetch_mut(ui) else {
            tracing::error!("button blocker widget is missing");
            return;
        };
        branch.set_visibility(!enable);
