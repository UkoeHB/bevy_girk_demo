...
fn max_page(lobby_contents: &ClickLobbyContents, member_type: ClickLobbyMemberType) -> usize
{
    ...
    if num_dangling_entries == 0 {
        num_pages = num_pages.saturating_sub(1);
    }
...
{
    let Some(lobby_contents) = lobby.get() else {
        return true;
    };
...
)
{
    ...
    for (idx, content_entity) in content_entities.iter().enumerate() {
        ...
        if member_number > lobby_contents.max(member_type) as usize {
            continue;
        }
    ...
    let new_page = match lobby.get() {
        None => 0,
        Some(lobby_contents) => match first_idx < lobby_contents.max(member_type) as usize {
            true => new_page,
            false => max_page(&lobby_contents, member_type),
        },
    };
    member_list_page.get_mut(&mut c).set(new_page);
}
...
)
{
    ...
    *last_lobby_id = current_lobby_id;
    ...
    // note: it would be more efficient to use world access directly here, but you can't have Locals in exclusive
    // systems
    commands.add(move |world: &mut World| {
        syscall(
            world,
            (content_entities, next_list_page),
...
)
{
    ...
    let button_entity = spawn_basic_button(ui, &area, "<", move |world: &mut World| {
        syscall(
            world,
            (
...
fn add_display_list_contents<ListPage: ListPageTrait>(ui: &mut UiBuilder<MainUi>, area: &Widget)
{
    ...
    for i in 0..NUM_LOBBY_CONTENT_ENTRIES {
        // text in center
        let y_range = (5. + (i as f32) * entry_height, 5. + ((i + 1) as f32) * entry_height);
    ...
    ui.commands().react().on(
        ...
        prep_fncall(
            ...
            update_display_list_contents_on_lobby_display::<ListPage>,
        ),
    );

    // add page left/right buttons
    ui.div_rel(area.end(""), (1., 15.), (85., 98.), |ui, area| {
        add_page_left_button::<ListPage>(ui, area, &contents)
    });
    ui.div_rel(area.end(""), (85., 99.), (85., 98.), |ui, area| {
        add_page_right_button::<ListPage>(ui, area, &contents)
    });
}
...
fn add_lobby_display_list<ListPage: ListPageTrait>(ui: &mut UiBuilder<MainUi>, area: &Widget)
{
    ...
    ui.div_rel(
        ...
        (0., 100.),
        (20., 100.),
        add_display_list_contents::<ListPage>,
    );
}
...
fn add_lobby_display_box(ui: &mut UiBuilder<MainUi>, area: &Widget)
{
    ...
    ui.div_rel(
        ...
        (0.5, 50.),
        (20., 100.),
        add_lobby_display_list::<PlayerListPage>,
    );
    ui.div_rel(
        area.end(""),
        (50., 99.5),
        (20., 100.),
        add_lobby_display_list::<WatcherListPage>,
    );
}
...
fn add_leave_lobby_button(ui: &mut UiBuilder<MainUi>, area: &Widget)
{
    ...
    ui.commands().add(move |world: &mut World| {
        syscall(
            world,
            (button_entity, default_text),
            setup_simple_pending_button_text::<LeaveLobby>,
        );
    });
}
...
fn add_start_game_button(ui: &mut UiBuilder<MainUi>, area: &Widget)
{
    ...
    ui.commands().add(move |world: &mut World| {
        syscall(
            world,
            (button_entity, default_text),
            setup_simple_pending_button_text::<LaunchLobby>,
        );
    });
}
...
pub(crate) fn add_lobby_display(ui: &mut UiBuilder<MainUi>, area: &Widget)
{
    ...
    ui.commands()
        .react()
        .trigger_resource_mutation::<LobbyDisplay>();
    ui.commands()
        .react()
        .trigger_resource_mutation::<PlayerListPage>();
    ui.commands()
        .react()
        .trigger_resource_mutation::<WatcherListPage>();
}
