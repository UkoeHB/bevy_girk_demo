...
)
{
    ...
    let Ok(target_entity) = reconnect_button.get_single() else {
        tracing::error!("ignoring reconnect game request because a request is already pending");
        return;
    };

    // sanity checks
    if monitor.is_running() {
        tracing::error!("reconnect game selected but client is currently in a game");
    }
    if !starter.has_starter() {
        tracing::error!("reconnect game selected but client cannot reconnect");
        return;
    }
    let Some(game_id) = starter.game_id() else {
        tracing::error!("starter game id missing on reconnect request");
        return;
    };
...
fn add_background(ui: &mut UiBuilder<MainUi>, area: &Widget)
{
    ...
    ui.commands()
        .spawn((barrier_img, UiInteractionBarrier::<MainUi>::default()));
}
...
fn add_reconnect_button(ui: &mut UiBuilder<MainUi>, area: &Widget)
{
    ...
    ui.commands().react().on(
        (
            resource_mutation::<ClientMonitor>(),
        ...
        move |mut ui: UiUtils<MainUi>, monitor: ReactRes<ClientMonitor>, starter: ReactRes<ClientStarter>| {
            ...
            ui.builder.add_style(
                ui.builder
                    .style::<GameInProgressStyle>()
                    .reconnect_button_avail
                    .clone(),
            );
    ...
    ui.commands().add(move |world: &mut World| {
        syscall(
            ...
            setup_simple_pending_button_text::<ReconnectorButton>,
        );
    });
}
...
pub(crate) fn add_game_in_progress(ui: &mut UiBuilder<MainUi>)
{
    ...
    ui.commands()
        .react()
        .trigger_resource_mutation::<ClientMonitor>();
    ui.commands()
        .react()
        .trigger_resource_mutation::<ClientStarter>();
}
